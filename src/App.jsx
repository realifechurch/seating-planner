...
export default function SeatingPlanner() {
  ...
  const canvasRef = useRef(null); // Reference for the download tool
    ...
    return (
      <div className="h-screen w-screen bg-slate-900 flex items-center justify-center">
        <form onSubmit={handleLogin} className="bg-slate-800 p-8 rounded-xl shadow-2xl w-96 border border-slate-700">
          ...
          <button type="submit" className="w-full bg-indigo-600 hover:bg-indigo-500 text-white font-bold p-3 rounded">ENTER SYSTEM</button>
        </form>
      </div>
    );
  }
    ...
    <div className="flex h-screen w-screen bg-slate-900 text-slate-200 overflow-hidden font-sans">
      ...
      <div className="w-80 bg-slate-800 p-6 flex flex-col border-r border-slate-700 shadow-2xl z-20">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-sm font-black text-indigo-400 uppercase">Guest List</h2>
          <button onClick={() => supabase.auth.signOut()} className="text-[10px] bg-slate-700 px-2 py-1 rounded">LOGOUT</button>
        </div>

        <label className="w-full bg-indigo-600 hover:bg-indigo-500 text-center p-2 rounded cursor-pointer font-bold text-xs mb-4 shadow-lg">
          + APPEND CSV
        ...
        <div className="flex-1 overflow-y-auto space-y-2 pr-1 border-t border-slate-700 pt-4"
          ...
          {unassigned.map(name => (
            <div key={name} draggable onDragStart={e => { e.dataTransfer.setData("name", name); e.dataTransfer.setData("source", "sidebar"); }}
                 className="p-3 bg-slate-700 border border-slate-600 rounded text-sm cursor-grab hover:bg-slate-600">
              {name}
            </div>
          ))}
        </div>
        ...
        <div className="mt-4 pt-4 border-t border-slate-700 space-y-2">
          <input value={planName} onChange={e => setPlanName(e.target.value)} placeholder="Event Name..." className="w-full bg-slate-900 border border-slate-700 p-2 rounded text-sm text-white" />
          <button onClick={saveToCloud} className="w-full bg-emerald-600 p-2 rounded font-bold text-sm hover:bg-emerald-500">SYNC TO CLOUD</button>
          <button onClick={downloadImage} className="w-full bg-slate-100 text-slate-900 p-2 rounded font-bold text-sm hover:bg-white transition">DOWNLOAD LAYOUT (PNG)</button>
          <button onClick={() => {
          ...
          }} className="w-full bg-slate-700 p-2 rounded text-[10px] opacity-60">+ ADD TABLE (MAX 30)</button>
        </div>
      </div>
      ...
      <div className="flex-1 flex flex-col bg-slate-900 p-4">
        ...
        <div className="flex justify-between items-center mb-4 px-6 py-4 bg-slate-800 rounded-xl border border-slate-700 shadow-xl">
          <div className="flex gap-10">
            <div className="flex flex-col"><span className="text-[10px] text-slate-400 uppercase font-bold">Total</span><span className="text-2xl font-black text-white">{stats.total}</span></div>
          ...
          <div className="text-right">
            <span className="text-[10px] text-slate-400 uppercase font-bold">Capacity Status</span>
            <div className="text-xl font-black text-white">{stats.fullTables} <span className="text-xs text-red-500">FULL TABLES</span></div>
          </div>
        </div>

        {/* CANVAS (REF ATTACHED HERE) */}
        <div ref={canvasRef} id="canvas-root" className="relative flex-1 bg-white rounded-xl overflow-hidden shadow-2xl border-4 border-slate-800" onDragOver={e => e.preventDefault()}>
          <div className="absolute left-0 h-full w-20 bg-slate-800 flex items-center justify-center pointer-events-none">
            <h1 className="text-white text-3xl font-serif tracking-widest uppercase -rotate-90 opacity-40">Stage</h1>
          </div>
          
          {Object.entries(tablePos).map(([id, pos]) => {
            const isFull = (tables[id]?.length || 0) >= 8;
            return (
              <div key={id} draggable
                ...
                className={`absolute flex flex-col items-center p-3 rounded-full border-2 shadow-lg cursor-move bg-slate-50 ${isFull ? 'border-red-600 ring-4 ring-red-100' : 'border-slate-300'}`}
                style={{ left: `${pos.x}%`, top: `${pos.y}%`, width: '180px', height: '180px', transform: 'translate(-50%, -50%)', zIndex: 10 }}>
                <span className={`font-black text-sm mb-1 ${isFull ? 'text-red-600' : 'text-slate-800'}`}>T{id} {(tables[id]?.length || 0)}/8</span>
                <div className="grid grid-cols-2 gap-1 w-full px-1">
                  {(tables[id] || []).map((name, i) => (
                    <div key={i} className="text-[8px] font-bold truncate bg-white border rounded p-1 text-center text-slate-700 shadow-sm">{name}</div>
                  ))}
                </div>
              </div>
            );
