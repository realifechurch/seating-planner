  ...
  const chairs = useMemo(() => {
    ...
    if (config.shape === 'circle') {
      ...
      for (let i = 0; i < capacity; i++) {
        ...
        items.push({ x: Math.cos(angle) * radius, z: Math.sin(angle) * radius, rotation: -angle + Math.PI / 2, guest: guests[i] });
      }
    } else {
      ...
      for (let i = 0; i < capacity; i++) {
        let cx = 0, cz = 0, rot = 0;
        if (currentDist < width) { cx = (currentDist - width / 2); cz = -(depth / 2) - 0.6; rot = 0; }
        else if (currentDist < width + depth) { const sideDist = currentDist - width; cx = (width / 2) + 0.6; cz = (sideDist - depth / 2); rot = -Math.PI / 2; }
        else if (currentDist < (width * 2) + depth) { const sideDist = currentDist - (width + depth); cx = (width / 2) - sideDist; cz = (depth / 2) + 0.6; rot = Math.PI; }
        else { const sideDist = currentDist - ((width * 2) + depth); cx = -(width / 2) - 0.6; cz = (depth / 2) - sideDist; rot = Math.PI / 2; }
        items.push({ x: cx, z: cz, rotation: rot, guest: guests[i] });
        currentDist += spacing;
      }
    }
    return items;
  }, [config, width, depth, seated, isDecor]);
    ...
    <group position={[x, 0, z]}>
      ...
      <Html position={[0, 2.5, 0]} center distanceFactor={10}>
        <div className="text-slate-800 font-bold text-xs bg-white/80 px-2 rounded-full border border-slate-200">
            {isDecor ? (config.type === 'plant' ? '' : config.type) : `T-${id}`}
        </div>
      </Html>

      {/* Main Body */}
      <mesh position={[0, tableHeight, 0]} castShadow receiveShadow>
        {config.shape === 'circle' ? ( <cylinderGeometry args={[width / 2, width / 2, 0.05, 64]} /> ) : ( <boxGeometry args={[width, 0.05, depth]} /> )}
        <meshStandardMaterial color={isDecor ? '#e2e8f0' : '#ffffff'} roughness={0.9} metalness={0.0} />
      </mesh>
      ...
      {chairs.map((chair, i) => ( <Chair key={i} {...chair} /> ))}
    </group>
  );
}