...
export default function Stage3D({ tables, tablePos }) {
  // CRITICAL FIX: Ensure these are always objects, handling undefined OR null
  const safeTables = tables || {};
  const safeTablePos = tablePos || {};

  // Extra safety: Check if data is ready before rendering
  const tableIds = safeTables ? Object.keys(safeTables) : [];
  ...
  return (
    <div className="w-full h-full bg-slate-900 absolute top-0 left-0 z-0">
      <Canvas shadows camera={{ position: [0, 40, 40], fov: 45 }}>
        ...
        {/* Use the safe list of IDs */}
        {tableIds.map((tableId) => {
          const config = safeTablePos[tableId];
          const guests = safeTables[tableId] || [];
          if (!config) return null;
          return <Table3D key={tableId} id={tableId} config={config} seated={guests} />;
        })}

        <OrbitControls minPolarAngle={0} maxPolarAngle={Math.PI / 2.2} minDistance={10} maxDistance={100} />
      </Canvas>
    </div>
  );
}