...
import TableNode from './TableNode'; // <--- RESTORED THIS IMPORT
...
export default function Stage({ 
  ...
  moveGuest, 
  deleteTable,
  addTable, // Added missing prop
  updateTableShape, // Added missing prop
  updateTableCapacity, // Added missing prop
  conflictTableIds = [], 
  viewScale = 1, 
  setViewScale
}) {
    ...
    <div className="flex-1 h-full relative overflow-hidden bg-[#F5F5F7]" onPointerMove={handlePointerMove} onPointerUp={handlePointerUp}>
      ...
      >
        {tableIds.map(id => {
            ...
            // Dimensions Logic
            // We use the dimensions from config, or reasonable defaults for the shapes
            ...
            return (
                ...
                >
                    {/* RESTORED: Using TableNode Component */}
                    {/* This component handles the visual shape (Circle/Rect) and renders GuestChair (Full Names) */}
                    <TableNode 
                        id={id}
                        config={config}
                        seated={guests}
